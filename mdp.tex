...
no history-dependent scheduler could differentiate the probabilities of paths satisfying
$\phi$ from neighbors. Such a definition would allow schedulers to
take different actions from different states. Two neighbors might be
differentiated by different action sequences erroneously.
A query scheduler $\scheduler{Q} : \bbfZ^{\geq 0} \rightarrow \omega$
corresponds to a query sequence. In differential privacy,
neighbors exhibit similar action behaviors.
