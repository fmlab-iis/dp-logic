...
In general, the problem is non-trivial. We consider the formula $\phi:=F B$ with $B\subseteq S$ and assume that states in $B$ are absorbing (with only self-loops). For the case $\epsilon=0$, the condition reduces to
$\myPr{s}{M_{\scheduler{S}}}{\neighbor{S}}{F B} -
...
becomes a metric version of the language equivalence problem, see \cite{FengZ13} for the definition. To the best of our knowledge, it has not be investigated in the literature. Thus we leave the general model checking algorithm as future works. \lz{see this}
