...
question. We have $AP=\{Y,N\}$, $L(Y)=\{Y\}$, $L(N)=\{N\}$, and $L(+)=L(-)=\emptyset$. \lz{draw it on the picture?}
...
In the differentially privacy context, we differentiate between offline and online models. 
...
Below we describe an online model. 
...
\lz{tbd, the complex threshold example we discussed, by Depeng}
