...
question. We have $AP=\{Y,N\}$, $L(Y)=\{Y\}$, $L(N)=\{N\}$, and $L(+)=L(-)=\emptyset$. States $+$ and $-$ are neighbors.\lz{In general, we omit a special state $\dagger$ with $L(\dagger)=\emptyset$, and all omitted transitions in the picture lead to this state.}
