...
As an example, for our threshold model(Algorithm ~\ref{algorithm:online-model}) , we consider the property $\wedge_{i=1}^{6} \dpriv{3}{0}(\X^i \top) \wedge \PJ{\le 0.025}(\X^6 F \top) \wedge \dpriv{3}{0.025}(\X^6 F \top)$, to express the privacy. The initial state of the corresponding Markov chain, where the input threshold $t$ is $3$, and the database $d$ is $4$, satisfies the above property by our computation. It means the probability of seeing $\top$  on any particular position, within $6$ steps, is $(3, 0)$-differentially private for the database $d$ and its neighbours($3$ and $5$). It also states that the probability of seeing $\top$ after no less than $6$ steps, is smaller than $0.025$ and is  $(3, 0.025)$-differentially private.
