...
3\myPr{v}{K}{N}{\X \mathit{out}_1}$ for the probability of satisfying
...
\item $\dpriv{\ln 3}{0} (\X (\mathit{out}_1\wedge \dpriv{\ln 3}{0}(\X \mathit{out}_1)))$. This is a nested $\dpCTL$ formula, where the inner state formula $\dpriv{\ln 3}{0}(\X \mathit{out}_1)$ specifies the one-step differential privacy. Observe the inner formula is satisfied by all neighbors. Thus, the outer formula also has no privacy loss.
\end{itemize}
