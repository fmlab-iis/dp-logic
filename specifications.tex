...
The formula $\dpriv{\ln 3}{0} (\X \mathit{out}_Y)$ \lz{$\X Y$ looks a bit weired?} holds in state $s$ if for  $t$ with $sNt$, the probability of satisfying $\X Y$ from $s$ and $t$ satisfy:
...
Now we consider the formula $\dpriv{\epsilon_2}{\delta_2} (\X (Y\wedge
\X Y))$ under the model in Figure~\ref{figure:2-dp-mc}. Applying the
...
As an example, for our threshold \lz{to do} model, we consider the property $\wedge_{i=1}^{c} \dpriv{\epsilon}{0}(\X^k \top) \wedge \PJ{\le 10^{-6}}(\X^c F T) \wedge \dpriv{\epsilon}{0}(\X^c F \top)$.
