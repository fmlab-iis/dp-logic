...
threshold only disclose minimal information and hence 
...
and queries $\{ f_i : f_i $ with sensitivity $\Delta
...
mechanism is applied to its result $r_i$. If the perturbed result
$r'_i$ is not less
than the perturbed threshold $t'$, the mechanism halts with the input
...
by entering the state of a self loop. 
...
Actions make sure the two computation on neighbors
